...
# 1. 스트림릿 페이지 기본 설정
st.set_page_config(
    page_title="전국 시군구 고령화 지도", page_icon="👵", layout="wide"
)

st.title("👵 전국 시군구 고령화지도")
st.caption(
    "2015~2026년 인구 데이터 중 가장 최신 연도를 기준으로 계산한 시군구별 65세 이상"
    " 인구 비율입니다."
)
...
# 3. Plotly 최신 API(px.choropleth_map)를 활용한 지도 작성
color_discrete_map = {
    "19% 미만": "#fef0d9",
    "19% 이상 ~ 23% 미만": "#fdcc8a",
    "23% 이상 ~ 28% 미만": "#fc8d59",
...
# 최신 버전에 맞춰 choropleth_map으로 수정되었습니다.
fig = px.choropleth_map(
    ...
    zoom=6.2,
    map_style="open-street-map",  # 타일 스타일 수정
    labels={
        "고령화율_구간": "고령화율 구간",
        "고령화율": "고령화율(%)",
        "시도": "시도",
